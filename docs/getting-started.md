...
```blade
<x-ichava::icon name="ichava/icon-sets-flag::<ratio>/<name>" class="w-6 h-6" />
```

The component is core's and is generic: the pack is named in the path, not in the tag. Which
`ratio` values this pack accepts is the subject of [Variants](variants.md).

## Or from PHP

```blade
{{ ichava('ichava/icon-sets-flag::<ratio>/<name>')->class('w-5 h-5') }}
```
...
```bash
php artisan ichava::ichava-core.info icons --package=ichava/icon-sets-flag
```
