...
```bash
php artisan vendor:publish --tag=ichava::icon-sets-flag-config
```

That writes `config/icon-sets-flag.php`, read back as `ichava.icon-sets-flag`. The defaults are merged without publishing,
